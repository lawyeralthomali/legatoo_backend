...
try:
    ...
    # Force import from installed package, not local file
    spec = importlib.util.find_spec("easyocr")
    if spec and spec.origin and "arabic_pdf_extractor.py" not in spec.origin:
        easyocr_lib = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(easyocr_lib)
